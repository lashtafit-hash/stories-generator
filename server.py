...
SCENARIOS = {
    ...
    "expert": [
        ...
        {
            "name": "Разбор кейса",
            "instruction": "Разбери реальный или типичный кейс: с чем пришёл клиент, где была проблема, что оказалось ключевым, какой получился результат. Пиши конкретно, без воды. Структура: Исходная точка — Что было важно понять — Что изменило результат — Вывод."
        },
        {
            "name": "FAQ без воды",
            "instruction": "Возьми частый вопрос клиента и ответь на него коротко, честно и сильно. Не размазывай. Структура: Вопрос — Короткий ответ — Что важно понять на самом деле — Вывод кому это подходит."
        },
        {
            "name": "До покупки люди путают вот это",
            "instruction": "Покажи тонкую, но важную путаницу клиента до покупки. Например: цена vs ценность, процедура vs результат, модно vs подходит. Структура: Что путают — Почему это ошибка — Как я смотрю на это как специалист."
        }
    ]
}
...
SCENARIO_MATCHERS = {
    "life": {
        "POV: Напряжение": ["очеред", "кофе", "подруж", "день", "сегодня", "магаз", "дорог", "устала", "быт"],
        "Внутренняя кухня мысли": ["дума", "осознал", "поняла", "мысл", "заметила", "почувствовала"],
        "Слом фасада": ["ожидани", "реаль", "идеаль", "на самом деле"],
        "Список Анти-дел": ["никогда", "больше не", "не делаю", "мои правила"],
        "Чит-код дня": ["лайфхак", "почему люблю", "мой способ", "секрет", "удобно"],
        "За vs Против": ["или", "vs", "против", "сравн", "выбираю"],
        "Эстетика момента": ["утро", "кофе", "вечер", "атмосфера", "эстет", "момент"]
    },
    "warmup": {
        "AIDA: Взлом внимания": ["новая процедура", "рассказать", "показать", "запуск", "ввела"],
        "PAS: Соль на рану": ["боль", "проблема", "не получается", "устали", "раздражает"],
        "До vs После": ["до", "после", "результат", "изменени", "эффект"],
        "Скрытая выгода": ["почему люблю", "неочевид", "за что люблю", "плюс"],
        "Фильтр для своих": ["не для всех", "кому не подойдет", "мой клиент"],
        "Цена бездействия": ["если тянуть", "потом", "теряете", "упускаете"],
        "Миф и Реальность": ["миф", "правда", "боятся", "думают", "ошибка"]
    },
    "sell": {
        "Последний вагон": ["места", "последний", "запись", "окно", "осталось"],
        "Математика выгоды": ["цена", "сколько стоит", "выгод", "деньги"],
        "Ожидание vs Результат": ["что будет после", "результат", "после процедуры"],
        "Отработка последнего НЕТ": ["боюсь", "страшно", "дорого", "нет времени", "возраж"],
        "Инвестиция в статус": ["дорого", "уровень", "статус", "ухоженно"],
        "Твёрдый результат": ["кейс", "результат", "клиент", "эффект"],
        "Проверка на прочность": ["выбор", "или дальше", "готовы", "смелость"]
    },
    "expert": {
        "Честный Хит-парад": ["топ", "список", "3", "5", "подборка"],
        "Битва Титанов": ["сравн", "или", "vs", "против", "лучше"],
        "Экспертный Лайфхак": ["как сделать", "секрет", "лайфхак", "быстро"],
        "Разбор Дичи": ["ошибка", "не надо", "неправильно", "бесят"],
        "Анатомия Качества": ["как понять", "на что смотрю", "качеств", "отличить"],
        "Мои стандарты": ["я не делаю", "мой стандарт", "почему я не", "никогда"],
        "Прогноз": ["тренд", "будущее", "через полгода", "куда идет"],
        "Разбор кейса": ["кейс", "клиент", "случай", "пришла", "результат"],
        "FAQ без воды": ["вопрос", "правда ли", "можно ли", "сколько", "больно ли"],
        "До покупки люди путают вот это": ["путают", "разница", "в чем отличие", "что выбрать"]
    }
}
...
Правитель, Эстет, Опекун, Искатель, Невинный, Бунтарь, Мудрец, Славный малый, Маг.
